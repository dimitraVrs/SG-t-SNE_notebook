...
md" Select input data from the above sections : "

# ╔═╡ 3a8f1ce5-ac95-4bf4-8564-9c3a6aa8f203
@bind dataChoice Select(["size","topology","distance","ellipse","parallel","noise","random circle","circle","same size","trefoil knot"])
...
begin
	...
	if cmp(dataChoice,"size")==0
		sectionData="sizeData"
	elseif cmp(dataChoice,"topology")==0
		sectionData="topData"
	elseif cmp(dataChoice,"distance")==0
		sectionData="distData"
	elseif cmp(dataChoice,"ellipse")==0
		sectionData="shapesData1"
	elseif cmp(dataChoice,"parallel")==0
		sectionData="shapesData2"
	elseif cmp(dataChoice,"random circle")==0
		sectionData="random-circle"
	elseif cmp(dataChoice,"circle")==0
		sectionData="circle"
	elseif cmp(dataChoice,"same size")==0
		sectionData="same_size"
	elseif cmp(dataChoice,"trefoil knot")==0
		sectionData="topData2"
	end
...
#@bind iterations PlutoUI.Slider(1000:500:3000)
...
function callExecDistr()

	option1="-l$lamda"
	option2="-m$iterations"
	#path="/home/dimitra/alg/bin/demo_stochastic_matrix" #= replace path with the path of the executable =#
	path=string(notebookDir,"/alg/bin/demo_stochastic_matrix")
	name=sectionData
	ext="mtx"

	#println("data for the alg. : $name")
	#---------------!!! execute algorithm !!!--------------
	
	try
                      run(pipeline(`$path $option1 $option2 $name.$ext`, stdout=stderr))
	catch excp
...
begin
	...
	yS=readdlm("embedding.txt",' ', '\n') 		
	#= y is pointsSizeX(dimensions+1) Matrix
	with the points after each iteration of the algorithm =#

	rm("embedding.txt")

	#println("I have read .txt file for $nameTest")
end

# ╔═╡ f3500552-aa3a-4a3f-9169-b99e61f0f2da
#pointsTest = MatrixMarket.mmread("$(sectionData).mtx")

# ╔═╡ 3384ef43-89a8-41e2-b895-1d2b58470c81
#nzTest=SparseArrays.nnz(pointsTest)

# ╔═╡ cb795b94-c680-46ec-bc70-d6b9751c488e
function showPlots(y,graph)
		#=-----------!!! create animation for the points 
					after each iteration of the algorithm !!!--------=#
	
		points = MatrixMarket.mmread("$(graph).mtx")
		pointsSize=size(points,1)
		nz=SparseArrays.nnz(points)
	
		anim=@animate for i in 2:50:iterations
			
				if(cmp(graph,"shapesData1")==0||cmp(graph,"noiseData")==0)
					scatter(y[(i-1)*pointsSize+1:i*pointsSize,1],y[(i-1)*pointsSize+1:i*pointsSize,2],label="step=$i",markercolor=:blue)
				elseif (cmp(graph,"sizeData")==0||cmp(graph,"topData")==0||cmp(graph,"shapesData2")==0)
					scatter(y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷2-1,1],y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷2+1,2],label="step=$i",markercolor=:blue)

					scatter!(y[(i-1)*pointsSize+1+nz÷2:(i-1)*pointsSize+nz÷2+nz÷2,1],y[(i-1)*pointsSize+1+nz÷2:(i-1)*pointsSize+nz÷2+nz÷2,2],label="step=$i",markercolor=:orange)
				else
					scatter(y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷3-1,1],y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷3+1,2],label="step=$i",markercolor=:blue)

					scatter!(y[(i-1)*pointsSize+1+nz÷3:(i-1)*pointsSize+nz÷3+nz÷3,1],y[(i-1)*pointsSize+1+nz÷3:(i-1)*pointsSize+nz÷3+nz÷3,2],label="step=$i",markercolor=:orange)

					scatter!(y[(i-1)*pointsSize+nz÷3+nz÷3+1:(i-1)*pointsSize+nz÷3+nz÷3+nz÷3+nz%3,1],y[(i-1)*pointsSize+nz÷3+nz÷3+1:(i-1)*pointsSize+nz÷3+nz÷3+nz÷3+nz%3,2],label="step=$i",markercolor=:green)
				end
				
				#=if (cmp(data,"mobius-graph")==0||cmp(data,"pbmc-graph")==0)
					if (dimensions==2)
					scatter(y[(i-1)*pointsSize+1:i*pointsSize,1],y[(i-1)*pointsSize+1:i*pointsSize,2],label="step=$i",markercolor=:blue)
					else
					scatter(y[(i-1)*pointsSize+1:i*pointsSize,1],y[(i-1)*pointsSize+1:i*pointsSize,2],y[(i-1)*pointsSize+1:i*pointsSize,3],label="step=$i")
					end
				end=#
		...
		if (cmp(graph,"shapesData1")==0||cmp(graph,"noiseData")==0)
					frame(anim,scatter(y[(i-1)*pointsSize+1:i*pointsSize,1],y[(i-1)*pointsSize+1:i*pointsSize,2],label="step=$i",markercolor=:blue))
		elseif (cmp(graph,"sizeData")==0||cmp(graph,"topData")==0||cmp(graph,"shapesData2")==0)
					scatter(y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷2-1,1],y[(i-1)*pointsSize+1:(i-1)*pointsSize+1+nz÷2+1,2],label="step=$i",markercolor=:blue)
		...
		#=if (cmp(data,"mobius-graph")==0||cmp(data,"pbmc-graph")==0)
				if (dimensions==2)
					...
					frame(anim,scatter(y[(i-1)*pointsSize+1:i*pointsSize,1],y[(i-1)*pointsSize+1:i*pointsSize,2],y[(i-1)*pointsSize+1:i*pointsSize,3],label="step=$i"))
				end
						end=#
...
begin
		restart
		mp4(showPlots(yS,sectionData),"$(sectionData)_l$(lamda)_iter$(iterations).mp4",fps=1,show_msg=false)
end
